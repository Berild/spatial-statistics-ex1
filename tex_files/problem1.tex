...
\textcolor{red}{The variogram function is}
%
\begin{align*}
\gamma_r(\tau)
...
\end{align*}
%
i.e. the variance minus the covariance. This is displayed for the given parameter values in figure \ref{fig:variograms}.
...
\end{equation}
with $I$ being the indicator function that is $1$ for $r(x)>2$ and $0$ otherwise.

We will use the posterior model for $\{r(x); x \in \text{D}\}$ problem 1d)-e) where the $\sigma^2_{\epsilon} = 0$ and consider two estimators for $A_r$. One predictor is made from the $100$ simulated realizations, $r_i(x)$ in problem 1e),
\begin{equation*}
    \hat{A}_r = \frac{1}{100}\sum_{i=1}^{100} \left(\sum_{x \in L} I(r_i(x)>2)\right),
\end{equation*}
...
\textbf{Also provide prediction interval for the first prediction} 
